...
# --- 1. Core Logic (PD 수정 시 즉시 반영되도록 max 제거) ---
def calc_unit_net(mode, tc, cu_p, cu_a, cu_py, cu_rc, cu_dt, cu_dv, 
    ...
    v_cu_pay = (cu_payable_content / 100.0) * (cu_p - (cu_rc * lb_to_mt))
    
    # 2. Ag (max 제거하여 PD가 Assay보다 커도 차이가 계산됨)
    if ag_dt == "PD":
        ag_payable_content = (ag_a * (ag_py / 100.0)) - ag_dv
    else:
        ag_payable_content = ag_a * (ag_py / 100.0 - ag_dv / 100.0)
    v_ag_pay = (ag_payable_content * g_to_oz) * (ag_p - ag_rc)
    ...
    v_au_pay = (au_payable_content * g_to_oz) * (au_p - au_rc)
...
# --- 5. Calculation (화면에 그리기 전에 계산을 먼저 완료) ---
res = {}
for _, k, _ in cases:
    res[k] = calc_unit_net(
...
# --- 6. Result Dashboard (계산된 res 값을 사용하여 출력) ---
